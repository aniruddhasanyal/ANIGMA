...
import validators
import web2text
# from web2text import getwebtxt
...
class RakeKeywordExtractor:
  ...
  def _calculate_word_scores(self, phrase_list):
    ...
    for phrase in phrase_list:
      degree = len((lambda x: not isNumeric(x), phrase)) - 1
      for word in phrase:
        word_freq[word] += 1
  ...
  def extract(self, text, incl_scores=False):
      ...
      sorted_phrase_scores = sorted(six.iteritems(phrase_scores),
...
def test():
    ...
    all the considered types of systems and systems of mixed types.
      """, incl_scores=True)
    #   keywords = rake.extract('https://en.wikipedia.org/wiki/Machine_learning', incl_scores=True)
    print(len(keywords))
    print(keywords)
